...
import GHC.GNNUtils --- uoe  1/6
import Debug.Trace (trace) -- uoe 2/6
import GHC.Data.FastString (fsLit) -- uoe 3/6
...
data PredPair = PredPair { predobj :: Predictions, predbool :: Bool }  -- uoe 4/6
...
makeCorePair dflags gbl_id is_default_method dict_arity rhs
  ...
  = case inlinePragmaSpec inline_prag of
          ...
          -- begin uoe  5/6
          NoUserInlinePrag -> if (predbool predpair) -------- Do default behavior if no predictions are passed in at cmd line
                            ...
                            else (gbl_id, rhs) -- end uoe 5/6
          NoInline  {}     -> (gbl_id, rhs)
          Opaque    {}     -> (gbl_id, rhs)
          Inlinable {}     -> (gbl_id `setIdUnfolding` inlinable_unf, rhs)
          Inline    {}     -> inline_pair
  where
    ------------- Begin modification block, uoe 6/6
    srcfilename =  case srcspan of     ---- get the file name to check for inlining recommendation function names
                    RealSrcSpan span _ -> srcSpanFile span
                    UnhelpfulSpan str  -> fsLit "UnhelpfulSpan"  -- quick hack.
    srcspan = (n_loc (idName gbl_id)) 
    occname = idName gbl_id
    ...
    -------------- End modification block, uoe 6/6
